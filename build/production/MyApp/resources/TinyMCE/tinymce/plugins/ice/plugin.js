...
(function() {
	tinymce.create('tinymce.plugins.IcePlugin', {
		...
		init: function(ed, url) {
			...
			// dev url
			url = "packages/TinyMCE/resources/tinymce/plugins/ice";

			// production url
			url = "resources/TinyMCE/tinymce/plugins/ice"; // production url
				...
				tinymce.DOM.win.setTimeout(function() {
					changeEditor = new ice.InlineChangeEditor({
						...
						changeTypes: {
							insertType: {
								tag: self.insertTag,
								alias: self.insertClass
							},
							deleteType: {
								tag: self.deleteTag,
								alias: self.deleteClass
							}
						}
					}).startTracking();
